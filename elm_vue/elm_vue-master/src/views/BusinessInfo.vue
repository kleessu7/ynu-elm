...
const goodsId = route.params.businessId; // 正确获取路由参数
...
const totle=ref(0)
const price=ref(0)
// const BUSINESS_API_URL = `http://localhost:8080/goods/detail/${goodsId}`;
const fetchData = async () => {
  try {
    const response = await axios.get('http://localhost:8080/goods/list');
    ...
    price.value = foods.value[0].sellingPrice;  
} else {  
    price.value = 0; // 或者其他默认值  
} 
    }
...
const handleCreateOrder = async () => {
  try {
    ...
    // 在成功创建订单后跳转到订单页面
    await router.push({name: 'Order', params: {orderId: goodsId,totalAmount: totle.value}});
  } catch (error) {
...
const increaseQuantity = async (foodId: number) => {
         ...
         totle.value=price.value* num.value +3 
};
...
<template>
  ...
  <div class="w-full h-full ">
    ...
    <div v-if="business" class="w-full h-full mt-28 flex justify-center items-center text-base">
      <img src="../assets/img/sp04.png" class="w-72 h-72 rounded">
    </div>
      ...
      <li v-for="food in foods" :key="food.goodsId"
        ...
        <div class="flex box-content items-center">
          ...
          <div class="ml-1.5">
            ...
            <p class="text-2xl text-gray-500 mx-3 font-normal mt-1">&#165;{{ food.sellingPrice }}</p>
          </div>
        </div>
        <!-- 添加和减少按钮逻辑 -->
        <div class="w-18 flex justify-between items-center">
          ...
          <div>
            <i class="fa fa-minus-circle fa-2x mx-3 text-gray-500 cursor-pointer"
               @click="() => decreaseQuantity(food.goodsId)"></i>
          </div>
          <!-- 数量 -->
          ...
          <div>
            <i class="fa fa-plus-circle fa-2x mx-3 text-blue-500 cursor-pointer"
               @click="() => increaseQuantity(food.goodsId)"></i>
          </div>
        </div>
      </li>
    </ul>
    ...
    <div class="w-full h-28 fixed left-0 bottom-0 flex text-base">
      <div class=" bg-gray-700 basis-2/3 flex">
        ...
        <div>
          <p class="text-4xl text-white mt-4">&#165;{{ price* num +3 }}</p>
